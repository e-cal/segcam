...
class MouseEventListener(QWidget):
        ...
        if self.is_freeze_button_press(event.x(), event.y()):
            ...
            if mask_clicked is not None:
                if is_delete:
                    ...
                    # Set color based on previous mask's color
                    if self.masks:
                        prev_color = self.masks[-1].color
                        new_mask.color = MaskColor.get_next_color(prev_color)
                    else:
                        new_mask.color = MaskColor.BLUE
    ...
    def draw_masks(self, qp=None, draw_buttons=True):
        ...
        # Draw all non-selected masks first
        for mask in self.masks:
            if mask == self.active_mask or mask.active_mask is None:
                continue
        ...
        # Draw selected mask last so it appears on top
